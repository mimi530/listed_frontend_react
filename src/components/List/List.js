...
export default function List({ list, show, setLists, lists, setList }) {
    ...
    return ( <
        >
        <
        div className = "box bg-orange has-text-dark my-3 is-clickable"
        onClick = { show } > {
            list.items_count ?
            ( <
...

        {
            editMode ? ( <
                ...
                name = "name"
                className = "input has-text-weight-bold subtitle is-6 is-size-6"
                autoFocus / >
                <
                /p> <
                p className = "control" >
                ...
                <
                h2 className = "subtitle has-text-dark has-text-weight-bold mb-2" > { list.name } <
                /h2> <
                div >
                <
                i className = "fa fa-edit has-text-info click-dark mr-2"
                onClick = {
                    () => setEditMode(true)
                } > < /i> <
                i className = "fa fa-trash has-text-danger click-dark"
                onClick = { handleListDelete } > < /i> < /
                div > <
                /div>
            )
        }
        ...
        <
        div className = "mr-2 is-clickable"
        title = { i18n.t('add_user_button') }
        onClick = {
            () => setIsModal('addList')
        } >
        <
